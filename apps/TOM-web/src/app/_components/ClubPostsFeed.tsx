...
export function ClubPostsFeed({
  ...
  const deletePost = async (postId: string) => {
    if (pendingDeletePostId || pendingDeleteCommentId) return;
    if (!window.confirm('Постоо устгах уу?')) return;
    ...
    try {
      ...
      onError?.(
        error instanceof Error ? error.message : 'Пост устгаж чадсангүй.'
      );
    } finally {
  ...
  const deleteComment = async (postId: string, commentId: string) => {
    if (pendingDeletePostId || pendingDeleteCommentId) return;
    if (!window.confirm('Сэтгэгдлээ устгах уу?')) return;
    ...
    try {
      ...
      onError?.(
        error instanceof Error ? error.message : 'Сэтгэгдэл устгаж чадсангүй.'
      );
    } finally {
  ...
  return (
    <section className="space-y-4">
      {posts.map((post) => {
        ...
        const isDeletingPost = pendingDeletePostId === post.id;
              ...
              <div className="flex items-center gap-2">
                ...
                {canDeletePost ? (
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-full border border-[#ffd2d5] bg-white px-3 py-1 text-xs font-semibold text-[#de4a58] transition hover:bg-[#fff6f7] disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    <Trash2 className="h-4 w-4" />
                    {isDeletingPost ? 'Устгаж байна...' : 'Устгах'}
                  </button>
                ) : null}
              </div>
            </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          (Boolean(canModerate) ||
                            ...
                            <button
                              ...
                              className="text-[11px] font-semibold text-[#de4a58] transition hover:underline disabled:cursor-not-allowed disabled:opacity-60"
                            >
