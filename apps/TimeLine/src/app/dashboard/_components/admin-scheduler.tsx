...
import { addDays, formatMonthDayLabel, formatMonthLabel, formatWeekdayName, getMonthDays, getMonthStart, getWeekStart, getYearMonths, getYearStart, isDateBefore, isSameMonth, toIsoDate } from '@/app/dashboard/_lib/scheduler-date-utils'
...
import { buildLocalEvent, createDefaultForm, createFormFromEvent, createGroupEditMutationInput, createSchedulerMutationInput, getSelectionForEvent, getSelectionTimeRange } from '@/app/dashboard/_lib/scheduler-form-utils'
...
import type { DraftForm, MatrixAxisMode, MonthViewFilter, ScheduleEventMutationInput, SchedulerQueryResult, SchedulerViewMode, Selection } from '@/app/dashboard/_lib/scheduler-types'
import { buildYearSummaries, createYearCalendarSummary, createYearFilterTotals } from '@/app/dashboard/_lib/scheduler-year-utils'
import { useSchedulerMoveController } from '@/app/dashboard/_lib/useSchedulerMoveController'
...
import { SchedulerMoveDialog } from '@/app/dashboard/_components/scheduler-move-dialog'
import { SchedulerMoveDraftsPanel } from '@/app/dashboard/_components/scheduler-move-drafts-panel'
...
export function AdminScheduler() {
  ...
  const moveController = useSchedulerMoveController({ createScheduleEvent, deleteScheduleEvent, events, refetch, resetComposerState, rooms, setLocalEvents, setMutationError, updateScheduleEvent })
  ...
  const selectYearMonth = (date: Date) => { const monthStart = getMonthStart(date); setCalendarMonth(monthStart); setFocusedDate(monthStart); setWeekStart(getWeekStart(monthStart)) }
  const openYearMonthSchedule = (date: Date) => { selectYearMonth(date); setViewMode('month') }
  ...
  const handleSave = async () => {
    if (!primarySelection || !form || !form.title.trim()) return
    const preserveSelectionTime = batchCreateMode && selectedBlocks.length > 1
    const groupId = preserveSelectionTime ? `schedule-group-${crypto.randomUUID()}` : null
    if (!preserveSelectionTime && timeToMinutes(normalizeTimelineTime(form.endTime)) <= timeToMinutes(normalizeTimelineTime(form.startTime))) return setMutationError('Дуусах цаг эхлэх цагаас хойш байх ёстой.')

    try {
      setMutationError(null)
      if (editingEvent) {
        const groupedEvents = getEditGroupEvents(editingEvent)
        if (groupedEvents.length > 1) {
          const groupedEventIds = groupedEvents.map((event) => event.id)
          const inputsToUpdate = groupedEvents.map((event) => ({ event, input: createGroupEditMutationInput(event, form, editingEvent.groupId ?? null) }))
          const conflictMessage = inputsToUpdate.map(({ input }) => getConflictMessage(input, groupedEventIds)).find(Boolean)
          if (conflictMessage) return setMutationError(conflictMessage)
          for (const { event, input } of inputsToUpdate) {
            if (event.id.startsWith('local-')) {
              setLocalEvents((current) => current.map((localEvent) => localEvent.id === event.id ? { ...localEvent, ...input, groupId: input.groupId ?? undefined, dayOfWeek: input.daysOfWeek[0] ?? localEvent.dayOfWeek, date: input.date ?? undefined, instructor: input.instructor ?? undefined, validFrom: input.validFrom ?? undefined, validUntil: input.validUntil ?? undefined, notes: input.notes ?? undefined } : localEvent))
            } else {
              await updateScheduleEvent({ variables: { id: event.id, input } })
            }
          }
          await refetch()
          setDialogOpen(false)
        ...
        const input = createSchedulerMutationInput(primarySelection, form, { groupId: editingEvent.groupId ?? null })
        const conflictMessage = getConflictMessage(input, editingEvent.id)
        if (conflictMessage) return setMutationError(conflictMessage)
        if (editingEvent.id.startsWith('local-')) setLocalEvents((current) => current.map((event) => event.id === editingEvent.id ? { ...buildLocalEvent(primarySelection, form, { groupId: editingEvent.groupId ?? null }), id: editingEvent.id } : event))
        else await updateScheduleEvent({ variables: { id: editingEvent.id, input } })
        await refetch()
        setDialogOpen(false)
        resetComposerState()
        return
      ...
      const selectionsToCreate = preserveSelectionTime ? selectedBlocks : [primarySelection]
      const inputsToCreate = selectionsToCreate.map((selection) => ({ selection, input: createSchedulerMutationInput(selection, form, { forceWeekly: preserveSelectionTime, groupId, preserveSelectionTime }) }))
      const conflictMessage = inputsToCreate.map(({ input }) => getConflictMessage(input)).find(Boolean)
      if (conflictMessage) return setMutationError(conflictMessage)
      const failedSelections: Selection[] = []
      let lastError: unknown = null
      for (const { selection, input } of inputsToCreate) {
        try {
          await createScheduleEvent({ variables: { input } })
        } catch (createError) {
          failedSelections.push(selection)
          lastError = createError
        }
      }
      await refetch()
      if (failedSelections.length > 0) {
        const errorMessage = lastError instanceof Error ? lastError.message : 'Хуваарь хадгалж чадсангүй'
        setMutationError(failedSelections.length === selectionsToCreate.length ? errorMessage : `${selectionsToCreate.length - failedSelections.length}/${selectionsToCreate.length} хуваарь хадгалагдлаа. Амжилтгүй slot-уудыг дахин сонгоод хадгална уу: ${errorMessage}`)
        return
      }
      setDialogOpen(false)
      resetComposerState()
  ...
  return (
    <section className={cn('space-y-4 transition-[padding-right] duration-200', dialogOpen ? 'xl:pr-[26rem]' : 'pr-0')} onPointerLeave={stopDragging}>
      <div className="rounded-md border border-[#d1d1d1] bg-white shadow-sm dark:border-border dark:bg-card"><Tabs value={selectedRoomTab} onValueChange={setSelectedRoomTab} className="gap-0"><SchedulerToolbar onCreateSchedule={() => openCreateDialog(selectedBlocksRef.current, { staged: selectedBlocksRef.current.length > 1 })} rooms={rooms} selectedRoomCount={selectedRoomCount} selectionCount={selectedBlocks.length} setViewMode={setViewMode} viewMode={viewMode} /><div className="grid items-stretch gap-[5px] p-2 xl:grid-cols-[clamp(176px,18vw,212px)_minmax(0,1fr)]"><SchedulerCalendarSidebar calendarMonth={calendarMonth} focusedDate={focusedDate} isCalendarOpen monthDays={monthDays} onSelectDay={viewMode === 'year' || viewMode === 'month' ? selectYearMonth : updateWeekStart} onToday={() => { if (viewMode === 'year' || viewMode === 'month') selectYearMonth(new Date()); else updateWeekStart(new Date()) }} setCalendarMonth={setCalendarMonth} summaryContent={viewMode === 'year' ? <SchedulerYearInsightsPanel monthName={selectedYearSummary ? formatMonthLabel(selectedYearSummary.monthStart) : 'Сар сонгоно уу'} selectedYearSummary={selectedYearSummary} yearLabel={String(yearStart.getFullYear())} yearSummary={yearSummary} /> : null} viewMode={viewMode} weekStart={weekStart} /><div className={cn('h-full min-w-0 overflow-hidden rounded-2xl border border-[#e8e8eb] bg-white shadow-sm transition-all duration-500 ease-out dark:border-border dark:bg-card', weekMotionClass)}><div className="h-full overflow-auto">{viewMode === 'week' ? <SchedulerWeekView errorMessage={error?.message} events={events} loading={loading && !data} onContextMenu={handleContextMenu} onEditEvent={openEditDialog} onEventDrop={moveController.handleEventDrop} onPointerDown={handlePointerDown} onPointerEnter={handlePointerEnter} onPointerUp={handlePointerUp} rooms={rooms} selectedRoomTab={selectedRoomTab} selectedRoomView={selectedRoomView} selections={displaySelections} weekDates={weekDates} /> : viewMode === 'day' ? <SchedulerDayView errorMessage={error?.message} events={events} focusedDateIso={focusedDateIso} focusedDateLabel={focusedDateLabel} focusedDateTitle={focusedDateTitle} focusedDayOfWeek={focusedDayOfWeek} loading={loading && !data} onContextMenu={handleContextMenu} onEditEvent={openEditDialog} onEventDrop={moveController.handleEventDrop} onPointerDown={handlePointerDown} onPointerEnter={handlePointerEnter} onPointerUp={handlePointerUp} rooms={roomsForFocusedView} selections={displaySelections} /> : viewMode === 'month' ? <SchedulerMonthSummaryView calendarMonth={calendarMonth} focusedDate={focusedDate} matrixAxisMode={matrixAxisMode} monthSummaries={monthSummaries} monthFilter={monthFilter} monthFilterTotals={monthFilterTotals} onAxisModeChange={setMatrixAxisMode} onFilterChange={setMonthFilter} onSelectDate={(date) => { updateWeekStart(date); setViewMode('day') }} rooms={roomsForFocusedView} selectedRoomView={selectedRoomView} /> : <SchedulerYearOverviewView calendarMonth={calendarMonth} matrixAxisMode={matrixAxisMode} onAxisModeChange={setMatrixAxisMode} rooms={roomsForFocusedView} selectedRoomView={selectedRoomView} yearFilter={yearFilter} yearFilterTotals={yearFilterTotals} yearSummaries={yearSummaries} onFilterChange={setYearFilter} onSelectMonth={openYearMonthSchedule} />}</div></div></div></Tabs></div>
      <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><MousePointer2 className="h-3.5 w-3.5" /><span>Дарж эсвэл чирээд хуваарь үүсгэнэ</span><span>Cmd/Ctrl + дарвал олон slot, олон өрөө сонгож болно</span>{stagedMultiSelect && selectedBlocks.length > 0 && !dialogOpen && <Badge variant="outline">{selectedRoomCount} өрөө · {selectedBlocks.length} сонголт</Badge>}{stagedMultiSelect && selectedBlocks.length > 0 && !dialogOpen && <Button type="button" size="sm" className="h-7 bg-[#6264a7] px-3 text-xs hover:bg-[#5558a7]" onClick={() => openCreateDialog(selectedBlocksRef.current, { staged: true })}>Нэг дор үүсгэх</Button>}{stagedMultiSelect && selectedBlocks.length > 0 && !dialogOpen && <Button type="button" size="sm" variant="outline" className="h-7 px-3 text-xs" onClick={resetComposerState}>Цэвэрлэх</Button>}<SchedulerLegend color="bg-[#2564cf]" label="Хичээл" /><SchedulerLegend color="bg-[#7f5ccf]" label="Клуб" /><SchedulerLegend color="bg-[#c4314b]" label="Event" />{mutationError && <Badge variant="outline" className="text-destructive">{mutationError}</Badge>}</div>
      <SchedulerMoveDraftsPanel drafts={moveController.moveDrafts} onConfirm={(id) => void moveController.confirmDraft(id)} onDelete={moveController.deleteDraft} />
      <SchedulerMoveDialog canMoveGroup={moveController.canMoveGroup} conflictPreview={moveController.conflictPreview} onApply={(scope) => void moveController.applyMove(scope)} onClose={moveController.closePendingMove} onSaveDraft={moveController.saveConflictDraft} pendingMove={moveController.pendingMove} />
