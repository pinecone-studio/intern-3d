...
export function AdminScheduler() {
  ...
  const getConflictMessage = (input: ScheduleEventMutationInput, excludeEventId?: string | null, sourceOccurrence?: { dateIso: string; event: ScheduleEvent }) => { const conflict = findScheduleConflict({ ...input, excludeEventId, sourceOccurrence }, events, rooms); return conflict ? formatScheduleConflict(conflict) : null }
  ...
  const applyEventMove = async (scope: EventMoveScope) => {
    ...
    try {
      ...
      const movedInput = buildMovedInput(pendingMove, scope === 'occurrence' ? 'occurrence' : 'series'); const conflictMessage = getConflictMessage(movedInput, scope === 'occurrence' ? null : pendingMove.event.id, scope === 'occurrence' ? { dateIso: pendingMove.sourceDateIso, event: pendingMove.event } : undefined); if (conflictMessage) return setMutationError(conflictMessage)
  ...
  return (
    <section className={cn('space-y-4 transition-[padding-right] duration-200', dialogOpen ? 'xl:pr-[26rem]' : 'pr-0')} onPointerLeave={stopDragging}>
      ...
      <Dialog open={Boolean(pendingMove)} onOpenChange={(open) => { if (!open) setPendingMove(null) }}><DialogContent className="max-w-md"><DialogHeader><DialogTitle>Давтамжтай хуваарь зөөх</DialogTitle><DialogDescription>{pendingMove?.event.title} хуваарийг зөвхөн энэ удаагийн occurrence дээр өөрчлөх үү, эсвэл бүх давтагдах хуваарь дээр өөрчлөх үү?</DialogDescription></DialogHeader><DialogFooter className="gap-2 sm:justify-between"><Button type="button" variant="outline" onClick={() => setPendingMove(null)}>Болих</Button><div className="flex gap-2"><Button type="button" variant="outline" onClick={() => void applyEventMove('occurrence')}>Зөвхөн энэ удаа</Button><Button type="button" className="bg-[#6264a7] hover:bg-[#5558a7]" onClick={() => void applyEventMove('series')}>Бүх давталт</Button></div></DialogFooter></DialogContent></Dialog>
