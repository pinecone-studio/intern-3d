...
export function SchedulerCalendarSidebar({ calendarMonth, focusedDate, isCalendarOpen, monthDays, onSelectDay, onToday, setCalendarMonth, summaryContent, viewMode, weekStart }: SchedulerCalendarSidebarProps) {
  return (
    <aside className={cn(isCalendarOpen ? 'block' : 'hidden', 'self-stretch rounded-2xl border border-[#e5e7f3] bg-[#fbfbfe] p-3 shadow-sm dark:border-[#2c3149] dark:bg-[#171b27] xl:block')}>
      <div className="mb-3 flex items-center justify-between"><div><p className="text-sm font-semibold text-foreground">{formatMonthLabel(calendarMonth)}</p></div><div className="flex items-center gap-1"><Button type="button" variant="ghost" size="icon" className="h-7 w-7 rounded-full" onClick={() => setCalendarMonth((current) => addMonths(current, -1))} aria-label="Өмнөх сар"><ChevronLeft className="h-4 w-4" /></Button><Button type="button" variant="ghost" size="icon" className="h-7 w-7 rounded-full" onClick={() => setCalendarMonth((current) => addMonths(current, 1))} aria-label="Дараагийн сар"><ChevronRight className="h-4 w-4" /></Button></div></div>
