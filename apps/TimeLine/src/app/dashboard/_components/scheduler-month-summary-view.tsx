...
function SegmentCell({ events, focusedDate, onSelectDate, room, slotIndex, span, summaries }: { events: ScheduleEvent[]; focusedDate: Date; onSelectDate: (_date: Date) => void; room: Room; slotIndex: number; span: number; summaries: MonthSummary[] }) {
  const isFocusedSegment = summaries.some((summary) => isSameDate(summary.date, focusedDate))
  const isWeekendOnly = summaries.every((summary) => summary.isWeekend)
  return (
    <td colSpan={span} className={cn('h-[34px] overflow-visible border-b border-r border-[#edebe9] p-0 align-top dark:border-border/60', isFocusedSegment && 'bg-[#eef0ff] dark:bg-[#252b45]', isWeekendOnly && 'bg-[#fafbff] dark:bg-[#161a27]')}>
